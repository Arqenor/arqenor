//! Blocking receiver + async stream wrapper for the SENTINEL filter port.
...
use sentinel_driver_common::{KernelEvent, KernelMessage};
...
/// Connected client to the SENTINEL kernel driver's filter communication port.
///
...
impl DriverClient {
    /// Connect to the `\SentinelPort` filter communication port.
    ///
    ...
    pub fn connect() -> Result<Self, ClientError> {
        ...
        match handle {
            Ok(h) => {
                debug!("Connected to \\SentinelPort (handle = {h:?})");
                Ok(Self {
