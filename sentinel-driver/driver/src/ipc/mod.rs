//! IPC module — Filter Manager communication port (`\SentinelPort`).
//!
...
// ---------------------------------------------------------------------------
// Port name — `\SentinelPort` as a static UTF-16 literal
// ---------------------------------------------------------------------------
...
impl IpcPort {
    ...
    /// connection (the SENTINEL usermode agent).
    ///
    /// # Safety
...
///
/// SENTINEL is push-only (kernel → user), so we don't expect inbound messages.
