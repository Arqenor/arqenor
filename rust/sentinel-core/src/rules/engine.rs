...
pub fn evaluate(rule: &DetectionRule, event: &ProcessEvent) -> Option<Alert> {
    let matched = match &rule.condition {
        RuleCondition::ProcessCreate { image, cmdline, cmdline_any, parent } => {
            let image_ok = image.as_ref()
                .map(|p| p.matches(&event.image) || p.matches(&event.name))
                .unwrap_or(true);
            let cmdline_ok = cmdline.as_ref()
                .map(|patterns| patterns.iter().all(|p| p.matches(&event.cmdline)))
                .unwrap_or(true);
            let cmdline_any_ok = cmdline_any.as_ref()
                .map(|patterns| patterns.iter().any(|p| p.matches(&event.cmdline)))
                .unwrap_or(true);
            let parent_ok = parent.as_ref()
                .map(|p| event.parent_image.as_deref()
                    .map(|pi| p.matches(pi))
                    .unwrap_or(false))
                .unwrap_or(true);
            image_ok && cmdline_ok && cmdline_any_ok && parent_ok
        }
        RuleCondition::ProcessName { names } => {
            names.iter().any(|p| p.matches(&event.name))
        }
    };
