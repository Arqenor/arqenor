...
//! 2. **Field mapping** — translate SIGMA field names to SENTINEL's event model.
...
/// SENTINEL's internal naming (`image_path`, `cmdline`, etc.).  The SIGMA
...
///
/// Field names in `event` should use SENTINEL's internal names.
/// The evaluator maps SIGMA field names automatically via [`sigma_field_to_sentinel`].
...
fn evaluate_field_matcher(fm: &FieldMatcher, event: &EventFields, category: &str) -> bool {
    let sentinel_field = sigma_field_to_sentinel(&fm.field, category);
    let event_value = match event.get(sentinel_field) {
...
/// Map SIGMA standard field names to SENTINEL's internal event field names.
pub fn sigma_field_to_sentinel<'a>(sigma_field: &'a str, category: &str) -> &'a str {
