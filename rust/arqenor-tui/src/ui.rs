...
fn draw_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let title = Line::from(vec![
        Span::styled("  ▓▓ ARQENOR  ", Style::default().fg(C_HEADER).add_modifier(Modifier::BOLD)),
...
fn conn_risk_color(c: &arqenor_core::models::connection::ConnectionInfo) -> Option<Color> {
    use arqenor_core::models::connection::{ConnState, ListenRisk, listen_risk_severity, local_port};
