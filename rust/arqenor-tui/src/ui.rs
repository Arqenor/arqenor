...
use arqenor_core::models::connection::{ConnState, Proto};
use ratatui::{
...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    if app.action_menu || app.action_result.is_some() {
        draw_action_menu(f, app, area);
    }
}
...
fn draw_tabs(f: &mut Frame, app: &App, area: Rect) {
    let net_label = match &app.net.scan_state {
        ScanState::Scanning { .. } => " Network (scanning…) ".to_string(),
    ...
    let tabs = Tabs::new(titles)
        ...
        .highlight_style(Style::default().fg(C_HEADER).add_modifier(Modifier::BOLD))
...
fn draw_process_detail(f: &mut Frame, app: &App, area: Rect) {
    ...
    let mut lines: Vec<Line> = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!(" {}   ", info.pid), Style::default().fg(C_NORMAL)),
            Span::styled("PPID ", Style::default().fg(C_HEADER)),
            Span::styled(info.ppid.to_string(), Style::default().fg(C_NORMAL)),
        ]),
    ];
...
fn draw_sidebar(f: &mut Frame, app: &App, area: Rect) {
    ...
    let lines: Vec<Line> = vec![
        ...
        if app.filter.is_empty() {
            Line::from(vec![Span::styled("  Press /", Style::default().fg(C_DIM))])
        } else {
            Line::from(vec![
                Span::raw("  "),
...
fn draw_process_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let rows: Vec<Row> = rows_data
        ...
        .map(|(i, r)| {
            ...
            Row::new(vec![
                Cell::from(format!("  {}", r.info.pid)).style(Style::default().fg(C_DIM).bg(bg)),
                Cell::from(r.info.ppid.to_string()).style(Style::default().fg(C_DIM).bg(bg)),
                Cell::from(r.risk.label()).style(
                    Style::default()
                        .fg(color)
                ...
                Cell::from(bar).style(Style::default().fg(color).bg(bg)),
                Cell::from(format!("{}{}", name_prefix, r.info.name)).style(
                    Style::default()
                        .fg(name_fg)
                        .add_modifier(if is_sel {
                            Modifier::BOLD
                ...
                Cell::from(path_text).style(Style::default().fg(path_color).bg(bg)),
            ])
            .height(1)
        })
        .collect();
...
fn draw_process_tree(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let header_cells = vec![
        ...
        Cell::from("SCORE").style(Style::default().fg(C_HEADER).add_modifier(Modifier::BOLD)),
        Cell::from("NAME                         ")
            .style(Style::default().fg(C_HEADER).add_modifier(Modifier::BOLD)),
...
fn draw_persistence(f: &mut Frame, app: &App, area: Rect) {
    ...
    let rows: Vec<Row> = app
        ...
        .map(|(i, e)| {
            ...
            Row::new(vec![
                Cell::from(format!("  {:?}", e.kind)).style(Style::default().fg(C_INFO).bg(bg)),
                Cell::from(e.name.clone()).style(
                    Style::default()
                        .fg(fg)
                        .add_modifier(if is_sel {
                            Modifier::BOLD
                ...
                Cell::from(e.command.clone()).style(Style::default().fg(fg).bg(bg)),
                Cell::from(e.location.clone()).style(Style::default().fg(C_DIM).bg(bg)),
            ])
        })
        .collect();
...
fn draw_network(f: &mut Frame, app: &App, area: Rect) {
    ...
    let high = app
        ...
        .hosts
        .iter()
        .filter(|h| h.risk == HostRisk::High)
        .count();
    let medium = app
        .net
        .hosts
        .iter()
        .filter(|h| h.risk == HostRisk::Medium)
        .count();
    let low = app
        .net
        .hosts
        .iter()
        .filter(|h| h.risk == HostRisk::Low)
        .count();
        ...
        app.net
            ...
            .map(|(i, h)| {
                ...
                Row::new(vec![
                    ...
                    Cell::from(h.risk.label()).style(Style::default().fg(color).bg(bg)),
                    Cell::from(h.os.label()).style(
                        Style::default()
                            .fg(match h.os {
                    ...
                    Cell::from(ports_str).style(Style::default().fg(C_DIM).bg(bg)),
                ])
            })
            .collect()
    };
...
fn conn_risk_color(c: &arqenor_core::models::connection::ConnectionInfo) -> Option<Color> {
    use arqenor_core::models::connection::{
        listen_risk_severity, local_port, ConnState, ListenRisk,
    };
...
fn firewall_tag(c: &arqenor_core::models::connection::ConnectionInfo) -> &'static str {
...
fn draw_connections_sidebar(f: &mut Frame, app: &App, area: Rect) {
    use arqenor_core::models::connection::{ConnState as CS, Proto as P};

    let listen = app
        .connections
        .iter()
        .filter(|c| c.state == CS::Listen)
        .count();
    let established = app
        .connections
        .iter()
        .filter(|c| c.state == CS::Established)
        .count();
    ...
    let fw_blocked = app
        .connections
        .iter()
        .filter(|c| c.firewall_blocked == Some(true))
        .count();
...
fn draw_connections(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let mut sorted: Vec<&arqenor_core::models::connection::ConnectionInfo> =
    ...
    let rows: Vec<Row> = sorted
        ...
        .map(|(i, c)| {
            ...
            let base_state_color = match c.state {
                ...
                ConnState::TimeWait | ConnState::CloseWait => C_MED,
                _ => C_DIM,
            };
            ...
            Row::new(vec![
                ...
                Cell::from(name_for_pid(c.pid)).style(
                    Style::default()
                        .fg(if is_sel { Color::White } else { C_NORMAL })
                        .add_modifier(if is_sel {
                            Modifier::BOLD
...
fn draw_statusbar(f: &mut Frame, app: &App, area: Rect) {
    ...
    match app.tab {
        Tab::Processes => {
            spans.extend([
                ...
                Span::styled(
                    if app.tree_mode { " Flat" } else { " Tree" },
                    Style::default().fg(C_DIM),
                ),
            ]);
        }
    ...
    let bar = Paragraph::new(Line::from(spans)).style(Style::default().bg(Color::Rgb(10, 10, 15)));
        ...
        .map(|(i, alert)| {
            ...
            Row::new(vec![
                Cell::from(sev_label).style(
                    Style::default()
                        .fg(sev_color)
                        .add_modifier(if is_sel || alert.sev_ord >= 4 {
                            Modifier::BOLD
                        } else {
                            Modifier::empty()
                        })
