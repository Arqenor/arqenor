...
//!
//! Opens a single named session (`ARQENOR-ETW-v1`) and enables high-value
...
impl EtwConsumer {
    ...
    pub fn start() -> Result<(Self, Receiver<EtwEvent>), ArqenorError> {
        // Register the global sender before any event can arrive.
        let (tx, rx) = sync_channel::<EtwEvent>(2048);
        ETW_TX
            .set(tx)
            .map_err(|_| ArqenorError::Platform("ETW consumer already running".into()))?;
        ...
        if let Err(e) = start_result {
            if e.code() == ERROR_ALREADY_EXISTS.to_hresult() {
                ...
                return Err(ArqenorError::Platform(format!("StartTrace: {e}")));
            }
        }
        ...
        let trace_handle = {
            ...
            if h.Value == u64::MAX {
                return Err(ArqenorError::Platform(
        ...
        thread::Builder::new()
            ...
            .map_err(|e| ArqenorError::Platform(format!("spawn etw-processor: {e}")))?;
