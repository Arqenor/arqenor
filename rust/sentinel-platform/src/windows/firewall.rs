...
pub fn query_firewall_block_status(
    ports: &[u16],
) -> Result<Vec<FirewallPortStatus>, SentinelError> {
    if ports.is_empty() {
        return Ok(Vec::new());
    }
...
unsafe fn query_inner(ports: &[u16]) -> Result<Vec<FirewallPortStatus>, SentinelError> {
    let policy: INetFwPolicy2 =
        CoCreateInstance(&NetFwPolicy2, None, CLSCTX_INPROC_SERVER).map_err(|e| {
            SentinelError::Platform(format!("CoCreateInstance(NetFwPolicy2) failed: {e}"))
        })?;

    let rules: INetFwRules = policy.Rules().map_err(|e| {
        SentinelError::Platform(format!("INetFwPolicy2::Rules() failed: {e}"))
    })?;
    ...
    let enumerator = rules._NewEnum().map_err(|e| {
        SentinelError::Platform(format!("INetFwRules::_NewEnum() failed: {e}"))
    })?;

    let enum_var: IEnumVARIANT = enumerator.cast().map_err(|e| {
        SentinelError::Platform(format!("cast to IEnumVARIANT failed: {e}"))
    })?;
