...
fn collect_tcp_v4() -> Result<Vec<ConnectionInfo>, SentinelError> {
...
fn query_tcp_table() -> Result<Vec<u8>, SentinelError> {
    unsafe {
        let mut size: u32 = 0;
        ...
        if ret != NO_ERROR.0 {
            return Err(SentinelError::Platform(format!(
...
fn collect_udp_v4() -> Result<Vec<ConnectionInfo>, SentinelError> {
...
fn query_udp_table() -> Result<Vec<u8>, SentinelError> {
    unsafe {
        let mut size: u32 = 0;
