//! Bridge between the SENTINEL kernel driver and the detection pipeline.
//!
//! Connects to the `\SentinelPort` filter communication port, receives
...
//! `DetectionPipeline`.
//!
//! Requires the `kernel-driver` cargo feature and a loaded `sentinel_driver.sys`.
//!
//! # Architecture
//! ```text
//! sentinel_driver.sys (kernel)
//!   │ FltSendMessage
//!   ▼
...
use sentinel_driver_common::KernelEventKind;
...
pub async fn start_driver_bridge(
    ...
    let client = DriverClient::connect()
        .map_err(|e| SentinelError::Platform(format!("driver connect: {e}")))?;

    info!("Connected to SENTINEL kernel driver (\\SentinelPort)");
